...
void controllGrowLED();

// 식물 피로도를 감안해 생장 LED 를 14시간 쬐어주고, 밤이 될때 10시간은 쉬게 해줌
void controllGrowLED(String getTime) {
  ...
  if(hour == "08" && minute == "00" && (second == "00" || second == "01")){ // 혹시나 00초를 놓칠 수 있으므로 안전하게 01초까지 보장
    digitalWrite(growLED, HIGH);
    Serial.println("아침 8시가 되었으므로 식물 생장 LED 를 켭니다");
  }
  
  // 매일 밤 22시(10시)마다 LED 끄기
  if(hour == "22" && minute == "00" && (second == "00" || second == "01")){
